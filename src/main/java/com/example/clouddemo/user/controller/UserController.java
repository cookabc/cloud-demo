package com.example.clouddemo.user.controller;
...
@Api(value = "UserController", description = "用户管理相关Api")
public class UserController {
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "sort", value = "排序，格式为:property,property(,ASC|DESC)的方式组织，" +
                    "如sort=firstname&sort=lastname,desc", dataType = "String", paramType = "query")
    })
    public List<UserDto> findAll(Pageable pageable) {
        Page<User> page = this.userService.getPage(pageable);
        if (null != page) {
            return page.getContent().stream().map(UserDto::new).collect(Collectors.toList());
        }
